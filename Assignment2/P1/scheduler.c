...
typedef struct task_info {
  ...
  int wake_time;
  //   c. If the task is waiting for another task, which task is it
  //   waiting for?
  int task_blocking;
  // 	 d. Was the task blocked waiting for user input? Once you
  //   successfully read input, you will need to save it here so it can
  //   be returned.
  char input_read;
 ...
 * is what calls the scheduler initially (round-robin after), but you should
 * write a generic scheduler that can (and will) be invoked by other blocking
 * functions as well (task_sleep and task_readchar).
 */
...
void scheduler() {
	/*
  if (&tasks[current_task] == -1 && current_task != 0)
    // current_task is the end of the created tassk current_task = 1;
    if (&tasks[current_task] == -1 && current_task == 0) {
      // no tasks to run

      // Maybe update current time
      if nothing
        is going on
    }
...
void task_create(task_t *handle, task_fn_t fn) {
  ...
  *handle = index;
  ...
  // this gets the code running for test1.c by swaping the conext without a
  // scheduler
  swapcontext(&main_context, &tasks[index].context);

  // Swaps to context for execution then returns to exit_context to call
  // task_exit
  /*
  if (from_main) {
    swapcontext(&main_context,
                &tasks[current_task]
                     .context); // Swaps from main context to current task

    from_main = false;
  } else {
    swapcontext(
        &tasks[current_task].context,
        &tasks[current_task].context); // Swaps from current task back to main
